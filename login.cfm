...
<cfscript>
	...
	if (structKeyExists(form, "confirmUser")){
		...
		if(session.isUserLoggedIn){
			session.userId = application.authentication.getUserId(form.username, form.password);
			session.isAdmin = application.authentication.isUserAdmin(session.userId.id);
		}
	}
</cfscript>	
...
<main>
	<div class="center">
		<cfif structKeyExists(form, "confirmUser") and session.isUserLoggedIn eq true>
